...
| Prefix | When to use |
|--------|-------------|
| `feat/` | New cryptography feature or algorithm |
| `fix/` | Bug fix |
...
Example: `feat/aes-encryption`, `fix/hmac-verification`
...
```
feat: add XOR cipher encryption
fix: resolve HMAC key encoding issue
docs: add Caesar cipher concept page
```
...
feat: adds hashing     # not imperative mood ("adds" -> "add")
```
...
> A hash is like a meat grinder: put any data in, get a fixed-size fingerprint
> out. You can't reverse it, but the same input always gives the same result.

Bad:

> A cryptographic hash function is a deterministic one-way mapping from an
> arbitrary-length input domain to a fixed-length output codomain.
...
Open an issue on [GitHub](https://github.com/robcox2910/pycrypt/issues) --
we're happy to help!
